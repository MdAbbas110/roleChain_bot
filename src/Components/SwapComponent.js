...
const SwapComponent = ({openSwapModal, setOpenSwapModal}) => {
    ...
    const [selectedAssetFrom, setSelectedAssetFrom] = useState(walletAssets.find(asset => asset.symbol === '$CRIB'));
    ...
    const handleSwap = () => {
      ...
      const confirmSwap = async () => {
        ...
        const updatedAssets = walletAssets.map(asset => {
            if (asset.symbol === selectedAssetFrom.symbol) {
                if (selectedAssetFrom.symbol === '$CRIB') {
        ...
        try {
          await updateDoc(userRef, {
            ...
            ...(selectedAssetFrom.symbol === '$CRIB' && { balance: newBalance }),
          ...
          setSwaps(prevSwaps => [...prevSwaps, swapHistory]);
            ...
            setTimeout(() => {
                ...
                setSwapAmount('');
                setSelectedAssetFrom(walletAssets.find(asset => asset.symbol === '$CRIB'));
      ...
      const handleAssetSelection = (asset, type) => {
        if (type === 'from') {
          ...
          if (asset.symbol === '$CRIB') {
            const filteredAssets = walletAssets.filter(asset => asset.symbol !== '$CRIB');
      ...
      ? walletAssets.filter(asset => asset.symbol !== '$CRIB') 
      : walletAssets;
      ...
      const cancelSwap = () => {
        ...
        setSwapAmount('');
        setCalculatedSwapValue(0);
        setSelectedAssetFrom(walletAssets.find(asset => asset.symbol === '$CRIB')); // Reset to default 'from' asset
