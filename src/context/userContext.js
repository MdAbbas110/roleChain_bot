...
const specialCards = [
  {
    ...
    description: 'This is a special card that gives you special access benefits to some of the wallet features on Tracex',
    class: 'specials1',
  },
  {
    title: 'Early Access',
...
export const UserProvider = ({ children }) => {
  ...
  const [level, setLevel] = useState({ id: 1, name: "Elite", imgUrl: "/elite.webp" });
  ...
  const assets = [
    {symbol: '$CRIB', name: 'Tracex', balance: balance, icon: '/tracex.webp',price: 0.0004348},
  ...
  const fetchData = async (userId) => {
    ...
    // Fetch settings data
    const settingsDocRef = doc(db, 'settings', 'GvFAb7mdclmyzxv6B7Et'); // Replace with your actual document ID
    ...
    if (telegramUser) {
      ...
      try {
        ...
        const userData = {
          ...
          level: { id: 1, name: "Elite", imgUrl: "/elite.webp" },
  ...
  const updateUserLevel = async (userId, newTapBalance) => {
    let newLevel = { id: 1, name: "Elite", imgUrl: "/elite.webp" };

    if (newTapBalance >= 1000 && newTapBalance < 50000) {
      newLevel = { id: 2, name: "Master", imgUrl: "/masterr.webp" };
    } else if (newTapBalance >= 50000 && newTapBalance < 500000) {
      newLevel = { id: 3, name: "Grandmaster", imgUrl: "/grandmaster.webp" };
    } else if (newTapBalance >= 500000 && newTapBalance < 1000000) {
      newLevel = { id: 4, name: "Epic", imgUrl: "/epic.webp" };
    } else if (newTapBalance >= 1000000 && newTapBalance < 2500000) {
      newLevel = { id: 5, name: "Legend", imgUrl: "/legend.webp" };
    } else if (newTapBalance >= 2500000) {
      newLevel = { id: 6, name: "Mythic", imgUrl: "/mythic.webp" };
    }
  ...
  useEffect(() => {
    ...
    const maxToken = walletAssets.find(asset => asset.symbol === '$CRIB');
    ...
    if (maxToken && maxToken.balance !== newBalance) {
      ...
      setWalletAssets(prevAssets =>
        prevAssets.map(asset =>
          asset.symbol === '$CRIB' ? { ...asset, balance: newBalance } : asset
        )
      );
    }
